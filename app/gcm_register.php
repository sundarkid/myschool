...
if (!isset($_POST['token']) || isset($_POST['tokenr'])) {
    ...
    if ($postResult) {
        while ($row = mysqli_fetch_array($postResult)) {
            $posts[] = array('post_id' => $row['pid'], "message" => $row['description'], "name" => $row['name'], "title" => $row['title'], "image" => $row['image'], "date" => $row['time'], "url" => $row['url']);
        }
        // TODO uncomment send_push_notification() to send message to devices
        //var_dump($messages);
        //echo "<br>".json_encode($messages);
